...
jq empty plugins/cold-start/.codex-plugin/plugin.json
jq empty plugins/cold-start/.claude-plugin/plugin.json
claude plugin validate --strict .
ruby -e 'require "yaml"; YAML.safe_load(File.read(ARGV[0]).split(/^---\s*$\n/)[1])' \
  plugins/cold-start/skills/cold-start/SKILL.md
